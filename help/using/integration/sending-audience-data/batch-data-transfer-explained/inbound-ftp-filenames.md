...
>[!WARNING]
>
>FTP設定のサポートは徐々に段階的に段階的に廃止されています。 既存のFTP統合では、受信データファイルの取り込みは引き続きサポートされますが、新しい統合のために、Amazon S3を使用してオフラインデータをオンボードすることを強くお勧めします。 詳細は、[受信データファイルの Amazon S3 名とファイルサイズの要件](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md)を参照してください。
