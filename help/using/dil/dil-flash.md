...
[!UICONTROL Flash DIL] は、Audience Manager でビデオ再生データを操作するための [!DNL ActionScript] コードライブラリです。[!DNL Flash DIL] は、Adobe [!UICONTROL AppMeasurement] ライブラリが Analytics に渡す SWF コンテンツをキャプチャすることにより機能します。[!DNL Flash DIL] はデータを独立した [!UICONTROL DIL] JavaScript データ収集モジュールに送信します。この情報は Audience Manager に渡されます。[!UICONTROL Props] ファイルからキャプチャされたAnalytics データ （[!UICONTROL eVars]、[!DNL FLA]、イベントなど）は、特性または未使用のシグナルとしてAudience Managerで利用できます。
