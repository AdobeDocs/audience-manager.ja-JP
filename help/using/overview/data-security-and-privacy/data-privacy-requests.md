...
複数のデバイスにまたがる [!DNL CRM] ID や [!DNL cookie] ID などの宣言済み ID がデータプライバシー要求で送信された場合、[!DNL Audience Manager] は関連付けられたすべてのデバイス（1 つの宣言済み ID につき最大 100 個のデバイス）に対して必要な削除をおこないます。
...
* [!DNL Audience Manager] は、すべてのセグメントから、オプトアウトされた ID と最後のデバイス ID のセグメントを解除します。[!DNL CRM]
