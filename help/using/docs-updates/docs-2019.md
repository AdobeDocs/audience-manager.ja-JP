...
| トピック | 説明 |
|--- |----|
| [プロファイル結合ルールの強化](../features/profile-merge-rules/merge-rules-overview.md) | 以下の機能強化の一連がリリースされまし [!UICONTROL Profile Merge Rules]た。 <ul><li>最大100台のデバイスに対して、セグメント評価がバッチでサポートされるようになりました。</li><li>特性とセグメントの訪問者数のレポート精度が向上しました。</li><li>デバイス間IDを使用して生成されたバッチファイルの精度が向上しました。</li><li>各ルールの使用例をさらに詳しく説明し、ドキュメントを更新しました。 詳しくは、 [プロファイル結合ルールの一般的な使用例](../features/profile-merge-rules/merge-rule-targeting-options.md)、外部デバ [イスグラフの使用例](../features/profile-merge-rules/external-graph-use-cases.md)、およびプ [ロファイルリンクデバイスグラフの使用例を参照してください](../features/profile-merge-rules/profile-link-use-case.md)。</li></ul> |
...
| トピック | 説明 |
|--- |----|
| [HTTP Strict-Transport-Security](../overview/data-security-and-privacy/data-security.md#hsts) | cookieのハイジャック攻撃やプロトコルダウ [!DNL HTTP Strict-Transport-Security]ングレード攻撃から保護するWebセキュリティポリシーのサポートが追加されました。 |
...
| トピック | 説明 |
|--- |----|
| [宛先ホームページ](/help/using/features/destinations/destinations-home.md) | The [Destinations](https://bank.demdex.com/portal/destinations/all) landing page has a new look. 新しいレイアウトについて詳しくは、このページを参照してください。 |

## 2019 年 5 月 {#aam-may-updates}

| トピック | 説明 |
|--- |----|
| [Audience Manager API](/help/using/api/rest-api-main/rest-api-main.md) | アドビは、製品ドキュメントから[すべての API](/help/using/api/api-swagger-migration.md) を製品ドキュメントから Swagger API へ移行している過程です。移行した最新の 2 つの API は次のとおりです。 <ul><li>[アルゴリズムモデルAPI](https://bank.demdex.com/portal/swagger/index.html#/Algorithmic_Models_API)</li><li>[派生シグナルAPI](https://bank.demdex.com/portal/swagger/index.html#/Derived_Signals_API)</li></ul> |
