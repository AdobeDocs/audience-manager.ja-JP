...
| トピック | 説明 |
|--- |----|
| [Audience Marketplaceデータに関するインテリジェントな推奨、Adobe Sensei](../features/segments/trait-recommendations.md) | 特性レコメンデーションを使用すると、セグメントビルダーでセグメントを作成または編集する際に [、登録していないデータフィードから、追加の特徴に関するレコメンデーションを取得できる](../features/segments/segment-builder.md)[!UICONTROL Audience Marketplace] ようになりました。 推奨特性をセグメントに追加すると、ターゲットオーディエンスを増やすことができます。<br>  さらに、類似の特徴を見つけや [!UICONTROL Marketplace] すくし、データフィードのフィルターを容易にするために、ページのデザインを変更しました。 |
