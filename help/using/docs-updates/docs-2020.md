...
| [Inbound S3 filenames](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) and [inbound FTP filenames](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | ファイル名の構文で、定義と `DPID``DPID_TARGET_DATA_OWNER` フィールドの例を更新しました。 |
