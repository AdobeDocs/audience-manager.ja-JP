...
[!UICONTROL Trend Analysis] レポートは、14日間の間隔でのみ行グラフにデータを返します。この例では、レポートにはマッピングされた一連のセグメントについてのインプレッション、クリックスルー、コンバージョンのトレンドが表示されます。

![](assets/trend-analysis.png)

## サンプルのボリューム分析レポート {#sample-volume-analysis}

[!UICONTROL Volume Analysis] レポートは、選択した日付範囲の棒グラフにデータを返します。この例では、レポートにはマッピングされた一連のセグメントについてのインプレッション、クリックスルー、コンバージョンが、ボリュームを基準に表示されています。
