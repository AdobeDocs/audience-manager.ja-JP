...
未使用のシグナルは45日間保存され、その後破棄されます。 未使用のシグナルレポートには、過去10日間のデータが表示されます。
...
* どちらの例も、2つのキーと値のペア vとaを含む特性を示しています。 最初の式は次のように変換されます。特性には、値1 [!UICONTROL AND NOT]のキーvと値23のキーaが含まれています。 2 番目の式は、「値が 1 のキー v、および値が 23 以外のキー a を含む（contains key v with the value 1 [!UICONTROL AND] the key a with the value [!UICONTROL NOT EQUAL] 23）」となります。
